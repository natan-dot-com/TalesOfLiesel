...
pairResult Context::damageOnClick() {
	...
	if (currEnemyInstance.inflictDamage(dealedDamage)) {
...
pairResult Context::evokeFireball() {
	if (this->fireballExec.joinable()) {
		pairResult currResult;
		bool isError;

		this->fireballExec = std::thread(this->damageFireball(), currResult, isError);
...
bool Context::updateFireball() {
	...
	playerInstance.spentSoulCoins(currSoulCoins);
